...
# ===================================================================
#              USE 2 SUBNETS FROM 2 DIFFERENT AVAILABILITY ZONES
# ===================================================================
locals {
...
resource "aws_instance" "nginx-node" {
  ami                    = "ami-01936304bea4b25ae"    # NGINX AMI you created
  ...
  user_data = <<-EOF
    ...
    sudo cp -r /tmp/fruits-veg_market/frontend/* /usr/share/nginx/html/
  EOF
...
resource "aws_instance" "python-node" {
  count                  = 2
  ami                    = "ami-0fd3ab0418fa90a1c"     # Python AMI
  instance_type          = "t3.micro"
  subnet_id              = local.subnet_az2
  vpc_security_group_ids = ["sg-08d9b35ca9022bd30"]
  key_name               = "MasterClass2025"
  ...
  tags = {
    Name = "terraform-python-node-${count.index + 1}"
  }
}
...
resource "aws_instance" "java-node" {
  count                  = 2
  ami                    = "ami-043f483fe0bce59d1"     # Java AMI
  instance_type          = "t3.micro"
  subnet_id              = local.subnet_az2
  vpc_security_group_ids = ["sg-08d9b35ca9022bd30"]
  key_name               = "MasterClass2025"
  ...
  tags = {
    Name = "terraform-java-node-${count.index + 1}"
  }
}
...
resource "aws_lb" "alb" {
  ...
  subnets = [
    local.subnet_az1,  # eu-west-1c
    local.subnet_az2   # eu-west-1a
  ]
